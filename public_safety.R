...
qrt_pruns <- pruns_report %>%
  count(FY.Q, Incident_Type)%>%
  mutate(FY.Q = as.numeric(FY.Q))%>%
  arrange(desc(FY.Q))
...
qrt_fruns <- fruns_report %>%
  count(FY.Q, Incident_Description)%>%
  mutate(FY.Q = as.numeric(FY.Q))%>%
  arrange(desc(FY.Q))
